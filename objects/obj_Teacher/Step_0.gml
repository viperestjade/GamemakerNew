...
if (is_moving_automatically) {
    ...
    if (_dist > cutscene_move_speed) {
        ...
        if ((_dir >= 45) && (_dir < 135)) {
            sprite_index = spr_TeacherBW; // Walking Up
        }
        else if ((_dir >= 135) && (_dir < 225)) {
            sprite_index = spr_TeacherLW; // Walking Left
        }
        else if ((_dir >= 225) && (_dir < 315)) {
            sprite_index = spr_TeacherFW; // Walking Down
        }
        else {
            sprite_index = spr_TeacherRW; // Walking Right
        }
    }
    // If we have arrived
    else {
        ...
        sprite_index = spr_Teacher_Idle; // Revert to the idle sprite
    }
